...
/**
 * Pipeline systemd SRT→FFmpeg : si le service est actif, le flux est considéré utilisable.
 * La sonde `signalState` sur le fichier HLS peut rester « offline » alors que le SRT (ex. :6001)
 * reçoit déjà du signal — ne pas bloquer dashboard / enregistrement sur ce seul critère.
 */
...
export function buildStreamRowsWithMetaFromSrs(
  ...
  const mappedRows: StreamRowWithMeta[] = base.map((m) => {
    ...
    if (m.protocol === "ICECAST") {
      isLive = Boolean(icecastLiveByStreamId[m.streamId])
