...
import { THRESHOLDS } from "@/lib/health/thresholds"
...
export default function RecordingsPage() {
  ...
  const disk = useSystemStats()
  const diskWarning = disk.diskPercent >= THRESHOLDS.disk.warning
  const diskCritical = disk.diskPercent >= THRESHOLDS.disk.critical
  const diskBlocksStart = diskCritical || disk.diskPercent >= 100 - 15
  const diskBannerLevel = diskCritical ? "critical" : diskWarning || diskBlocksStart ? "warning" : "ok"
  ...
  return (
    <>
      <div className="space-y-6">
          ...
          actions={
            <>
              <Button
                ...
                disabled={diskBlocksStart}
                title={diskBlocksStart ? "Espace disque insuffisant" : undefined}
              >
                Record all
              </Button>
        ...
        <Card
          className={
            diskBannerLevel === "critical"
              ? "border-red-500/40 bg-red-500/10"
              : diskBannerLevel === "warning"
        ...
        >
          <CardContent className="flex flex-col gap-1 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-medium text-foreground">Stockage recordings</p>
              <p className="text-xs text-muted-foreground">
                {disk.loading
                  ? "Mesure disque…"
                  : disk.error
                    ? "Mesure disque indisponible"
                    : `${disk.diskPercent.toFixed(0)} % utilisés · Disponible : ${disk.diskFree.toFixed(0)} GB / ${disk.diskTotal.toFixed(0)} GB`}
              </p>
            </div>
            {diskBannerLevel !== "ok" && !disk.loading && !disk.error ? (
              <p className="text-sm font-medium text-amber-700 dark:text-amber-300">
                ⚠ Espace disque faible
                {diskBlocksStart ? " — démarrage d’enregistrement bloqué" : ""}
              </p>
            ) : null}
          </CardContent>
        </Card>
            ...
            sources.map((src) => {
              ...
              return (
                <RecordingStreamCard
                  ...
                  startBlocked={diskBlocksStart}
                  startBlockedReason="Espace disque insuffisant (seuil recordings)"
