...
export type StreamRow = {
  ...
  status: "LIVE" | "OFFLINE" | "REC" | "DEGRADED"
  bitrateKbps: number
  resolution: string
  fps: string
  clients: number
...
function statusBadge(status: StreamRow["status"]) {
  if (status === "LIVE") return <StatusBadge label="● LIVE" tone="live" />
  if (status === "REC") return <StatusBadge label="● REC" tone="rec" />
  if (status === "DEGRADED") return <StatusBadge label="● DEGRADED" tone="warn" />
  return <StatusBadge label="OFFLINE" tone="idle" />
}
