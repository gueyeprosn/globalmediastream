...
import { collectExternalTvPoints } from "@/lib/traffic/srt-relay"
import type {
  TrafficConnection,
  TrafficHlsBucket,
  TrafficPipeline,
...
function parseSystemdTimestamp(raw: string): { startedAt: string | null; uptimeSec: number | null } {
  const t = raw.trim()
...
export async function collectTrafficReport(): Promise<TrafficReport> {
  ...
  const uniqueIps = [
    ...new Set(
      rawClients
        .map((c) => String(c.ip || "").replace(/^::ffff:/i, "").trim())
        .filter(Boolean)
    ),
  ]
  ...
  const connections = rawClients.map((c) =>
    normalizeClient(c, nowMs, countryByIp.get(String(c.ip || "").replace(/^::ffff:/i, "")) ?? null)
  )
  ...
  const summary: TrafficSummary = {
    ...
    streamsLive: streams.filter((s) => s.publishActive).length + externalTv.filter((e) => e.signalLive).length,
    streamsTotal: streams.length + externalTv.length,
    bwInKbps: streams.reduce((a, s) => a + s.bwInKbps, 0) + externalRecvKbps,
    bwOutKbps: streams.reduce((a, s) => a + s.bwOutKbps, 0) + externalSendKbps,
