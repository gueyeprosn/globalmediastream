...
import { formatUptimeSec, parseSystemdTimestamp } from '@/lib/systemd-uptime'
...
interface StreamConfig {
  ...
  serverUrl?: string
  username?: string
  /** Mot de passe jamais exposé — flag uniquement. */
  hasPassword?: boolean
  genre?: string
  description?: string
  bitrate?: number
...
async function getServiceStatus(serviceName: string): Promise<{ status: string, uptime: string, cpu: number, memory: string }> {
  ...
  try {
    ...
    if (status === 'active') {
      try {
        const raw = await systemctlShowValue(unit, 'ActiveEnterTimestamp')
        const { uptimeSec } = parseSystemdTimestamp(raw)
        if (uptimeSec != null) uptime = formatUptimeSec(uptimeSec)
      } catch {
        /* ignore — uptime reste N/A */
      }
    }
    return { status, uptime, cpu: 0, memory: '0 MB' }
  } catch {
  ...
  try {
    ...
    for (const source of sources) {
      ...
      icecastStreams.push({
        ...
        inputUrl: `icecast://127.0.0.1:8000${normalizedMount}`,
    ...
    for (const [mountName, config] of mountPoints.entries()) {
      ...
      if (!exists) {
        icecastStreams.push({
          ...
          listeners: 0,
          listenerPeak: 0,
          inputUrl: `icecast://127.0.0.1:8000${mountName.startsWith('/') ? mountName : `/${mountName}`}`,
