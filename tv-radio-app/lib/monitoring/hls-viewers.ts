...
import { assertSafeNginxAccessLog, tailFileLines } from '@/lib/host-metrics'
...
export async function getHlsViewerDetailForStreams(
  ...
  const logPathRaw = process.env.NGINX_ACCESS_LOG || '/var/log/nginx/access.log'

  let text = ''
  try {
    const logPath = assertSafeNginxAccessLog(logPathRaw)
    text = await tailFileLines(logPath, 15_000)
  } catch {
    return result
  }
