...
**App** : Next.js 16.3.0 · PM2 `oceanfm-app` · port 3000 (loopback derrière Nginx)  
...
**Statut correction** : lots **1→11** déployés en prod (2026-08-09). Backups `.lotN-backup/`.

---

## Constat global

L’auth API via `proxy.ts` tient en prod ; `lib/safe-shell.ts` est solide **là où il est utilisé**. Les risques majeurs étaient les **contournements shell**, les **actions live sans confirmation**, le **disque recordings**, et la **dépendance exclusive au proxy** (surtout avec des CVE Next.js ouvertes).

**Correction** : plan lots 1–11 appliqué (voir tableau de suivi en bas de document).
...
| Statut | |
|--------|--|
| Corrigés | **30 / 30** (lots 1–11) |
| Décisions produit tranchées | **B9** DEGRADED (pipeline + HLS frozen/offline, SRS live prioritaire) · **C8** KPI External TV = 1 · **B1** cookie-only · **Icecast** `hasPassword` · **Next** 16.2.11 puis **16.3.0** |

---

## Suivi des lots (correction)

| Lot | Items | Statut |
|-----|-------|--------|
| 1 | A2, A5 confirm Kick/Cleanup | ✅ |
| 2 | A3 disque recordings + FFmpeg | ✅ |
| 3 | A1 ffprobe safe-shell, B5 kick timeout, B6 audit | ✅ |
| 4 | B3 metrics/health hors exec | ✅ |
| 5 | B1 cookie JWT, B2 Icecast secrets | ✅ |
| 6 | A4/B13 Next 16.2.11 + requireAuth | ✅ |
| 7 | B8 SIGKILL, C4–C7 | ✅ |
| 8 | B10–B12, C3/C12 partiel | ✅ |
| 9 | B4 rate-limit, B7 pages proxy, C11 hash, C1/C2 | ✅ |
| 10 | B9 DEGRADED, C8 KPI dédup | ✅ |
| 11 | Next 16.3.0 + `npm audit` 0 vulns, polish light mode, doc | ✅ |

### Procédure de déploiement (inchangée)
...
```bash
# Ex. lot N :
cp -a /srv/tv-radio-app/.lotN-backup/<fichier> /srv/tv-radio-app/<chemin>
cd /srv/tv-radio-app && npm run build && pm2 restart oceanfm-app --update-env
```
...
Aucune action bloquante restante sur le périmètre audit. Suivi ops : valider login (hash bcrypt), DEGRADED sur un flux HLS frozen, KPI `/traffic` pendant un External TV republished SRS.
